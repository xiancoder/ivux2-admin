        ...
        <div class="applicantInfoNode">
            ...
            <div class="applicant_data">
                ...
                <table style="margin-top: 10px" v-show="showAll === 1">
                    <tr v-if="info.oldUrl"> <td>原链接</td> <td>{{info.oldUrl}}</td> </tr>
                    <tr v-if="info.newUrl"> <td>新链接</td> <td>{{info.newUrl}}</td> </tr>
                    <tr v-if="info.oldPrice"> <td>原单价</td> <td>{{info.oldPrice}}</td> </tr>
                    <tr v-if="info.newPrice"> <td>新单价</td> <td>{{info.newPrice}}</td> </tr>
                    <tr v-if="info.oldPaymentType||info.oldPaymentType===0"> <td>原收款方式</td> <td>{{info.oldPaymentType===1?'对私':'对公'}}</td> </tr>
                    <tr v-if="info.newPaymentType||info.newPaymentType===0"> <td>新收款方式</td> <td>{{info.newPaymentType===1?'对私':'对公'}}</td> </tr>
                    <tr v-if="info.oldSettleTypeName"> <td>原结算周期</td> <td>{{info.oldSettleTypeName}}</td> </tr>
                    <tr v-if="info.newSettleTypeName"> <td>新结算周期</td> <td>{{info.newSettleTypeName}}</td> </tr>
                    <tr v-if="info.oldAccount"> <td>原后台账号</td> <td>{{info.oldAccount}}</td> </tr>
                    <tr v-if="info.newAccount"> <td>新后台账号</td> <td>{{info.newAccount}}</td> </tr>
                    <tr v-if="info.oldPassword"> <td>原后台密码</td> <td>{{info.oldPassword}}</td> </tr>
                    <tr v-if="info.newPassword"> <td>新后台密码</td> <td>{{info.newPassword}}</td> </tr>
                    <tr> <td>维护说明</td> <td>{{info.adDesc || '-'}}</td> </tr>
                    <tr> <td>维护人</td> <td>{{info.userName || '-'}}</td> </tr>
                    <tr> <td>维护时间</td> <td>{{info.createdTime || '-'}}</td> </tr>
                    <tr> <td>投放渠道</td> <td>{{info.whChannel || '-'}}</td> </tr>
                </table>

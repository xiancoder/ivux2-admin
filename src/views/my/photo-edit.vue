<template>
    <div id="demo" class="tailor">
        <!-- 遮罩层 -->
        <div class="container" >
            <div>
                <img id="image" :src="$route.query.url" alt="Picture">
            </div>
...
export default {
    ...
    methods: {
        crop () {
            this.$vux.loading.show({text: '正在上次头像'})
            this.$router.go(-1)
            var croppedCanvas
            var roundedCanvas
            if (!this.croppable) {
...
<style lang='less'>
    ...
    .tailor .cropper-modal { opacity: .5; background-color: #000;}
    ...
    .tailor .cropper-dashed { position: absolute;display: block; opacity: .5; border: 0 dashed #eee;}
    ...
    .tailor .cropper-center { position: absolute; top: 50%; left: 50%;display: block; width: 0; height: 0; opacity: .75; }
    .tailor .cropper-center:before, .cropper-center:after { position: absolute;display: block; content: ' '; background-color: #eee;}
    ...
    .tailor .cropper-face,
    .tailor .cropper-line,
    .tailor .cropper-point { position: absolute;display: block; width: 100%; height: 100%; opacity: .1;}
    ...
    .tailor .cropper-point { width: 5px; height: 5px; opacity: .75; background-color: #39f;}

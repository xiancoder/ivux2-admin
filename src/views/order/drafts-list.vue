                        ...
                        <tr v-for="row in response.list" :key="row.id" @click="toRelease(row.id, index)">
                            <td style="padding-top: 6px;text-align: left;">
                                <div :style="{width: tdWidth + 'px'}" class="overhide" style="display: flex;display: -webkit-flex;">
                                    ...
                                    <div class="imgDiv"><img src="../../img/delete.png" alt=""  @click.stop="del(index, row.id)"></div>
                                </div>
    ...
    methods: {
        ...
        toRelease (id, index) {
            if (id === -1) {
                ...
                this.$router.push({name: 'order-release', query: {id: id, type: '2'}})
            }
...
<style scoped lang='less'>
    ...
    .imgDiv{ flex: 1; text-align: right;
        img{ height: 16px; margin-left: 16px;}
    }
</style>

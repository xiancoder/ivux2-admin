<template>
    <div class="orderList">
        <x-header class="header_div" @on-click-back="goHome" :left-options="{backText: '', preventGoBack: true}"><span class="header_title">我的工单</span><img style="width: 25px;" @click="toRelease()" src="../../img/add-new.png" slot="right" alt=""></x-header>
        ...
        <div class="grey_search tab_search">
            <search class="searchInput" style="width: 100%" v-model="request.keyword" :auto-fixed="false"  placeholder="标题、内容、接收人、组" @on-change="debSearch" @on-submit="submiting"></search>
        </div>
                    ...
                    <tbody>
                        ...
                        <tr v-for="(row, index) in response.list" :key="row.id" @click="toDetail(row.id, index)">
                            <td style="padding-top: 6px;text-align: left;">
                                ...
                                <div v-if="row.repleName" :style="{width: tdWidth + 'px'}" class="overhide">最新回复：{{row.repleName}}</div>
                            </td>
                        </tr>
                    </tbody>
                </x-table>
...
import {Group, XHeader, TabItem, XTable, Scroller, LoadMore, Divider, Search, debounce} from 'vux'
export default {
    components: {Group, XHeader, TabItem, XTable, Scroller, LoadMore, Divider, Search, tab, noData},
    data () {
        return {
            names: [
                {name: '已收工单', router: 'receive-list'},
                {name: '已发工单', router: 'send-list'},
                {name: '草稿箱', router: 'drafts-list'},
                {name: '已结工单', router: 'end-list'}
            ],
            request: {
                keyword: '',
                pageIndex: 1,
    ...
    methods: {
        ...
        toRelease () {
            this.$router.push({name: 'order-release', query: {tab: '1'}})
        },
        // 置顶更新数据
        update () {
            removeOrder()
        ...
        toDetail (id, index) {
            this.$router.push({name: 'order-info', query: {id: id, handle: 1, ending: 1, send: 1, notice: 1}})

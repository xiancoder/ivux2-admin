...
<template>
    <group class="my-group">
        ...
        <tab active-color="#0094eb" :animate="false">
            ...
            <tab-item selected @on-item-click="onItemClick">我已审核的</tab-item>
    ...
    methods: {
        ...
        toDetail (id, type, wId, vType) {
            switch (wId) {
                case 7:
                    if (vType === '哺乳假') {
                        this.$router.push({name: 'breastfeed-info', query: {id: id, role: '3'}})
                    } else {
                        this.$router.push({name: 'vacation-info', query: {id: id, role: '3'}})
                    }
                    break
                case 8:
                    this.$router.push({name: 'overtime-info', query: {id: id, role: '3'}})
                    break
                case 9:
                    this.$router.push({name: 'noCheck-info', query: {id: id, role: '3'}})
                    break
                case 11:
                    this.$router.push({name: 'outwork-info', query: {id: id, role: '3'}})
                    break
                case 12:
                    this.$router.push({name: 'busTrip-info', query: {id: id, role: '3'}})
                    break
            }
        }
    },

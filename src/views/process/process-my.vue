...
<template>
    <group class="my-group">
        ...
        <div class="my-search-form">
            ...
            <search class="searchInput" style="width: 90%" v-model="keyword" :auto-fixed="false"  placeholder="模糊搜索事由" @on-change="debounceSearch" @on-submit="submiting()"></search>
        </div>
        <div>
            <popup class="search-pop" v-model="screen" position="left">
                <div style="width: 300px">
                    ...
                    <div class="pop-left">
                        ...
                        <div :class="condition === 4 ? 'left-item-active' : 'left-item'" @click="condition=4">审核状态</div>
                    </div>
                    ...
                    <div class="pop-right" v-show="condition === 4">
                        <checker class="my-check-item" radio-required  v-model="search.status" default-item-class="demo1-item" selected-item-class="demo1-item-selected">
                            <checker-item v-for="row in statusData" :value='row.id' :key="row.id">{{row.name}}</checker-item>
                        </checker>
                    </div>
                    ...
                    <x-table :cell-bordered="false" class="myprocessTable">
                        <tbody>
                            ...
                            <tr v-else v-for="(row, index) in dataArr" :key="index" @click="toDetail(row.id, row.type, row.wId, row.vacationType)">
                                <td class="textLef overhide" :width="tdWidth" style="padding: 5px 0;" :class="row.status === 4 ? 'grey' : ''">
                                    <div class="wid100">
                                        <p class="wid50 textLef" :class="row.statusColor">{{row.statusName}}</p>
    ...
    methods: {
        ...
        toDetail (id, type, wId, vType) {
            switch (wId) {
                case 7:
                    if (vType === '哺乳假') {
                        this.$router.push({name: 'breastfeed-info', query: {id: id, role: '1'}})
                    } else {
                        this.$router.push({name: 'vacation-info', query: {id: id, role: '1', processMy: 1}})
                    }
                    break
                case 8:
                    this.$router.push({name: 'overtime-info', query: {id: id, role: '1'}})
                    break
                case 9:
                    this.$router.push({name: 'noCheck-info', query: {id: id, role: '1'}})
                    break
                case 11:
                    this.$router.push({name: 'outwork-info', query: {id: id, role: '1'}})
                    break
                case 12:
                    this.$router.push({name: 'busTrip-info', query: {id: id, role: '1'}})
                    break
            }
        }
    },

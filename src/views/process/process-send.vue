                    ...
                    <x-table :cell-bordered="false">
                        <tbody>
                            ...
                            <tr v-else v-for="(row, index) in dataArr" :key="index" @click="toDetail(row.id, row.type, row.wId, row.vacationType)">
                                <td class="textLef overhide lineH26" :width="tdWidth" style="padding: 5px 0;" :class="row.status === 4 ? 'grey' : ''">
                                    <div class="wid100">
                                        <p class="wid50 textLef"><span :class="row.statusColor" style="margin-right: 10px">{{row.statusName}}</span>{{row.name}}</p>
    ...
    methods: {
        ...
        toDetail (id, type, wId, vType) {
            switch (wId) {
                case 7:
                    if (vType === '哺乳假') {
                        this.$router.push({name: 'breastfeed-info', query: {id: id, role: '4'}})
                    } else {
                        this.$router.push({name: 'vacation-info', query: {id: id, role: '4'}})
                    }
                    break
                case 8:
                    this.$router.push({name: 'overtime-info', query: {id: id, role: '4'}})
                    break
                case 9:
                    this.$router.push({name: 'noCheck-info', query: {id: id, role: '4'}})
                    break
                case 11:
                    this.$router.push({name: 'outwork-info', query: {id: id, role: '4'}})
                    break
                case 12:
                    this.$router.push({name: 'busTrip-info', query: {id: id, role: '4'}})
                    break
            }
        }
    },

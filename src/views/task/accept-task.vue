<template>
    <div>
        <x-header @on-click-back="goInfo" :left-options="{backText: '', preventGoBack: true}" style="background-color: #0094eb">接受任务<a slot="right" style="color: #fff" @click="saveAccept">保存</a></x-header>
        <div class="info_container">
            <group>
                <cell title="完成日期" primary="content"><datetime v-model="deadline" placeholder="必填" :readonly="deadline !== ''"></datetime></cell>
...
export default {
    name: 'accept-task',
    components: {
        Group, Cell, XHeader, Popup, Datetime, Search, Icon
    },
    data () {
        return {
            userList: [],
            ...
            if (this.deadline === '') {
                ...
                this.$post('api/task/accept', {
                    ...
                    if (res.data.data.res === 1) {
                        ...
                        this.$router.replace({name: 'task-info', query: {taskId: this.$route.query.taskId, role: '2'}})
                    } else {
                        this.$vux.alert.show({
                            title: '提示',

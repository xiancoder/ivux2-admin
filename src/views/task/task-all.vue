<template>
    <group class="my-group">
        <x-header @on-click-back="goHome" :left-options="{backText: '', preventGoBack: true}" style="background-color: #0094eb">下属任务</x-header>
...
import { Popup, Checker, XTable, LoadMore, Scroller, Tab, TabItem, Datetime,
    CheckerItem, Group, XHeader, Cell, Divider, XButton, XSwitch, Search } from 'vux'
export default {
    name: 'task-all',
    components: {
    ...
    methods: {
        ...
        onItemClick (index) {
            switch (index) {
                case 0: this.$router.push({name: 'my-task'}); break
                case 1: this.$router.push({name: 'my-response'}); break
                case 2: this.$router.push({name: 'my-operate'}); break
                case 3: this.$router.push({name: 'my-cc'}); break
            }
        },
        toInfo (id) {
            this.$router.push({name: 'task-info', query: {taskId: id, role: '5'}})
        },
        openP (mark) {
            this.modalP = true
            this.valueP = ''
        ...
        getList (page_size, v) {
            ...
            this.$get('api/taskunderling/all', pa).then((res) => {
